...
void SPIWrThenRd(unsigned char* Write, unsigned int writeL, unsigned char* Read,
				 unsigned int readL) {
	unsigned int n;
...
unsigned long CharsToFlashAddr(unsigned char addbyte1, unsigned char addbyte2,
							   unsigned char addbyte3) {
...
void SectorToFlashAddr(unsigned int sector, unsigned long* begin, unsigned long* end) {
	unsigned long temp;
...
void BlockToFlashAddr(unsigned int block, unsigned long* begin, unsigned long* end) {
	unsigned long temp;
...
void PageToFlashAddr(unsigned int page, unsigned long* begin, unsigned long* end) {
	unsigned long temp;
...
unsigned char CheckFlashEmpty(unsigned long address, unsigned int len) {
	unsigned char ReadBuf[FLASH_PAGE_SIZE + 1]; // Flash can only write a page at a time.  So this
...
void flashEraseBetweenBlocks(unsigned int start_block,
							 unsigned int end_block) // Erases blocks, inclusive
{
	unsigned int i;
...
void flashEraseBlockByAddr(unsigned long addr) {
	unsigned char WriteBuf[4]; // max write is FLASH_PAGE_SIZE, need up to 6 bytes for flash command
							   // and address
	unsigned char ReadBuf[4];
...
void flashGlobalUnprotect() {
	unsigned char WriteBuf[4]; // max write is FLASH_PAGE_SIZE, need up to 6 bytes for flash command
							   // and address
	unsigned char ReadBuf[4];
	const unsigned char opcode = FLASH_WRITE_STATUS_REGISTER_BYTE1; // Write the status byte1
	const unsigned char status_register = B8(
		00000000); // This bit combination unlocks all sectors and keeps the protection bit unlocked
...
void flashCopyBlockExceptPage(unsigned int destination_block, unsigned int source_block,
							  unsigned int page_to_except) {
...
ReturnErr_t flashWriteSafe(unsigned long addr, int len, unsigned char* buf,
...
ReturnErr_t flashFindEmptyPage(unsigned int startpage, unsigned int endpage,
...
void flashErasePage(unsigned int page) {
	...
	unsigned long start_addr, end_addr; // Addresses for swap block
...
unsigned long rolls over the top (sorta)
...
void DecrementAddrCircular(unsigned long* addr, unsigned long lower_limit,
						   unsigned long upper_limit, unsigned long decrement) {
	// Bail if invalid input
...
{
	unsigned char CopyBuf[FLASH_PAGE_SIZE];	// Cannot be replaced with global ReadBuf because of
