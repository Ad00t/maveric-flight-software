...
/*=================== Compute CRC ========================
  PURPOSE: Creates a unique checksum for a specified length of uint8_t buffer.
  UNMODIFIED VARIABLES: len, buf
  MODIFIED VARIABLES: value
  GLOBALS: none
  RETURN VALUE: crc as uint16 
...
/*=================== Compute CRC Continuous ========================
  ...
  NOTES:
      ...
      as if the last uint8_t buffer and the current uint8_t buffer were concatenated.
-----------------------------------------------------------
*/
uint16_t compute_crc16_cont(uint8_t restart, uint8_t* buf, uint8_t len) {
    static uint16_t crc = 0x0000; // keep state across calls
    uint8_t i, j;

    if (restart == CRC_RESTART) {
        crc = 0x0000; // reset state
    }
...
uint32_t compute_crc32(uint8_t* data, int length) {
	...
	for (idx = 0; idx < length; idx++) {
		crc = CRC_TABLE[(crc ^ data[idx]) & 0xffL] ^ ((crc >> 8) & 0xFFFFFF);
	}
