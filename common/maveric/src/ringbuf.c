...
int1 rb_peek(ringbuf_s* buf, uint16_t off, uint8_t* out) {
    ...
    uint16_t i = (buf->r + off) % RINGBUF_MAX_SIZE;
    *out = buf->data[i];
    return 1;
}
...
int1 rb_push(ringbuf_s* buf, uint8_t b) {
    uint16_t i = (buf->w + 1) % RINGBUF_MAX_SIZE;
    if (i == buf->r) return 0;
    buf->data[buf->w] = b;
    buf->w = i;
    return 1;
...
int1 rb_pop(ringbuf_s* buf, uint16_t n, uint8_t* out) {
    ...
    if (out == NULL) {
        buf->r = (buf->r + n) % RINGBUF_MAX_SIZE;
    } else {
        size_t j;
        for (j = 0; j < n; j++) {
            out[j] = buf->data[buf->r]; 
            buf->r = (buf->r + 1) % RINGBUF_MAX_SIZE;
        }
    }
    return 1;
}
