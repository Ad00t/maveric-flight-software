...
void i2c1_read_word(unsigned int8 add, unsigned int8 reg, unsigned int8 *output, unsigned int8 num_bytes)
{
	...
	{
		*output = i2c_read(I2C_1);
		*(output++);	
	}
	*output = i2c_read(I2C_1, FALSE);
	i2c_stop(I2C_1);		
}
#endif

#ifdef I2C_2
unsigned int8 i2c2_read_8(unsigned int8 add, unsigned int8 reg)
{
	//I2C 1  byte reading routine
	//int8(i2c address), int8(i2c register)  -> int8
	//reads the bytes present in the i2c register of the addressed device and saves it on read_data 
	...
	i2c_start(I2C_2);
	i2c_write(I2C_2, addw);
	i2c_write(I2C_2, reg);
	//i2c_stop(I2C_1);
	i2c_start(I2C_2);
	i2c_write(I2C_2, addr);

	read_data = i2c_read(I2C_2, FALSE);
	//read_data = (read_data<<8);
	//read_data = read_data | i2c_read(I2C_1);
	i2c_stop(I2C_2);
...
void i2c2_write_8(unsigned int8 add, unsigned int8 reg, unsigned int8 write_data)
{
	//I2C writing byte routine
	//int8(i2c address), int8(i2c register), char(1 byte write register pointer)  -> none
	//Writes the bytes present in write_data into the i2c register of the addressed device 
...
unsigned int16 i2c2_read_16(unsigned int8 add, unsigned int8 reg)
{
	//I2C INA226 reading routine
	//int8(i2c address), int8(i2c register), -> int16
	//reads the bytes present in the i2c register of the addressed device and saves it on read_data 
	...
	read_data = i2c_read(I2C_2);
	read_data = (read_data<<8);
	read_data = read_data | i2c_read(I2C_2, FALSE);
	i2c_stop(I2C_2);

	return read_data;		
}

void i2c2_write_16(unsigned int8 add, unsigned int8 reg, unsigned int16 write_data)
{
	//I2C INA226 writing routine
	//int8(i2c address), int8(i2c register), char(2 bytes write register pointer)  -> bool
	//Writes the bytes present in write_data into the i2c register of the addressed device 
...
void i2c2_read_word(unsigned int8 add, unsigned int8 reg, unsigned int8 *output, unsigned int8 num_bytes)
{
	//I2C INA226 reading routine
	//int8(i2c address), int8(i2c register), -> int16
	//reads the bytes present in the i2c register of the addressed device and saves it on read_data 
	...
	for (i=0; i<num_bytes; ++i)
	{
		*output = i2c_read(I2C_2);
		*(output++);	
	}
	*output = i2c_read(I2C_2, FALSE);
	i2c_stop(I2C_2);		
}
#endif

#ifdef I2C_3
unsigned int8 i2c3_read_8(unsigned int8 add, unsigned int8 reg)
{
	//I2C 1  byte reading routine
	//int8(i2c address), int8(i2c register)  -> int8
	//reads the bytes present in the i2c register of the addressed device and saves it on read_data 
	...
	i2c_start(I2C_3);
	i2c_write(I2C_3, addw);
	i2c_write(I2C_3, reg);
	//i2c_stop(I2C_1);
	i2c_start(I2C_3);
	i2c_write(I2C_3, addr);

	read_data = i2c_read(I2C_3, FALSE);
	//read_data = (read_data<<8);
	//read_data = read_data | i2c_read(I2C_1);
	i2c_stop(I2C_3);
...
void i2c3_write_8(unsigned int8 add, unsigned int8 reg, unsigned int8 write_data)
{
	//I2C writing byte routine
	//int8(i2c address), int8(i2c register), char(1 byte write register pointer)  -> none
	//Writes the bytes present in write_data into the i2c register of the addressed device 
...
unsigned int16 i2c3_read_16(unsigned int8 add, unsigned int8 reg)
{
	//I2C INA226 reading routine
	//int8(i2c address), int8(i2c register), -> int16
	//reads the bytes present in the i2c register of the addressed device and saves it on read_data 
	...
	read_data = i2c_read(I2C_3);
	read_data = (read_data<<8);
	read_data = read_data | i2c_read(I2C_3, FALSE);
	i2c_stop(I2C_3);

	return read_data;		
}

void i2c3_write_16(unsigned int8 add, unsigned int8 reg, unsigned int16 write_data)
{
	//I2C INA226 writing routine
	//int8(i2c address), int8(i2c register), char(2 bytes write register pointer)  -> bool
	//Writes the bytes present in write_data into the i2c register of the addressed device 
...
void i2c3_read_word(unsigned int8 add, unsigned int8 reg, unsigned int8 *output, unsigned int8 num_bytes)
{
	//I2C INA226 reading routine
	//int8(i2c address), int8(i2c register), -> int16
	//reads the bytes present in the i2c register of the addressed device and saves it on read_data 
	...
	for (i=0; i<num_bytes; ++i)
	{
		*output = i2c_read(I2C_3);
		*(output++);	
	}
	*output = i2c_read(I2C_3, FALSE);
	i2c_stop(I2C_3);		
}
#endif