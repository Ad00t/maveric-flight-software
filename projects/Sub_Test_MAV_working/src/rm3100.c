...
{	
	unsigned int8 read_data[9];
	unsigned int8 num_bytes = 9;
	
 	rm3100_read_word(add, RM3100_REGISTER_MX2, read_data, num_bytes);
	if (read_data[0] > 0x80)
	{
		*mag_x = (int32)0xFF << 24;
	}
	...
	*mag_y += (int32)read_data[3] << 16;
	*mag_y += (int32)read_data[4] << 8;
	*mag_y += (int32)read_data[5];
	...
	*mag_z += (int32)read_data[6] << 16;
	*mag_z += (int32)read_data[7] << 8;
	*mag_z += (int32)read_data[8];
}