...
int1 ina226_init(unsigned int8 add)
{
	...
	rdata = i2c_read_16(add, INA226_REG_MANID);

	if (rdata ==0x5449)
	{
		rdata = i2c_read_16(add, INA226_REG_ID);
		if (rdata==0x2260)
		{
			return 1;
		}
					  ...
					  unsigned int16 setmode = INA226_MODE_8)
{
	...
	i2c_write_16(add, INA226_REG_CONFIG, ina226_config);
}
...
void ina226_calibrate(unsigned int8 add, unsigned int16 calibration = 0x1400)
{
 // For R_shunt = 1 mohm and 1 mA/bit resolution calibration = 0x1400
	i2c_write_16(add, INA226_REG_CALIBRATION, calibration);
}

void ina226_read_data(unsigned int8 add, int16* shunt_voltage, int16* bus_voltage, int16* power, int16* current)
{
	*shunt_voltage = 2.5 * i2c_read_16(add, INA226_REG_SHUNTV);
	*bus_voltage = 1.25 * i2c_read_16(add, INA226_REG_VBUS);
	*power = 25 * i2c_read_16(add, INA226_REG_PWR);
	*current = 1* i2c_read_16(add, INA226_REG_CURRENT);
}