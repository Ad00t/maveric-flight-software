...
uint16_t framer_create(uint8_t* msg, uint16_t msg_len, uint8_t* frame, int8_t csp) {
...
// KISS
int8_t kiss_process_byte(kiss_parser_s* parser, uint8_t msg_byte) {
    switch (parser->fsm) {
        case KISS_WAIT_FEND:
            if (msg_byte == FEND) {
                parser->buf_len = 0;
                parser->fsm = KISS_IN_FRAME;
            }
            break;
        ...
        case KISS_IN_FRAME:
            ...
            } else if (msg_byte == FESC) {
                parser->fsm = KISS_IN_ESCAPE;
            } else {
                if (parser->buf_len < FRAME_MAX_SIZE) {
                    parser->buf[parser->buf_len++] = msg_byte;
                } else {
                    // overflow -> drop frame
                    parser->fsm = KISS_WAIT_FEND;
                    parser->buf_len = 0;
                }
            }
            break;
